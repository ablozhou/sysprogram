...
void *process(void *data)
{
    ...
    if (data == NULL)
    {
        printf("Error run thread, no data.\n");
        return NULL;
    }
    Msgs *msgs = (Msgs *)data;
    // open client fifo
...
void main(int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        status = pthread_create(&thd, NULL, process, &msgs);
        if (status != 0)
        {
            printf("Error creating process thrread.\n");
            exit(1);
        }
        status = pthread_detach(thd);
        if (status != 0)
        {
            printf("Error: process thrread detach failed.\n");
            exit(1);
        }
    }
    exit(0);
